...
| `--coverpkg` | | Pass `-coverpkg` to `go test` to measure cross-package coverage (e.g. `./...`) | `""` (disabled) |
...
- The profile is huge — the parser is streaming, so large profiles (hundreds of MB) are handled without loading the whole file into memory.
...
- When `--coverage-profile` is set, `scan` analyzes the module the profile belongs to (the nearest `go.mod` ancestor of the profile file) rather than the current working directory, so coverage and complexity stay aligned even when run from elsewhere. An explicit `path` target wins when it resolves to a module.
...
Plain `go test` only records coverage for statements in the package under test
(the package whose `_test.go` exercises them). Functions in other packages that
are called by those tests show up as uncovered. Pass `-coverpkg` so `go test`
also counts statements in every package the tests touch:
...
The `--coverpkg` flag forwards the same value to the `go test -coverpkg` that
`scan` runs when no profile is supplied:

```shell
go-crap scan --coverpkg ./...
```
